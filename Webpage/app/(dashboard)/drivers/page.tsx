...
import { DriversResultsSkeleton } from "./loading";
...
export default async function DriversPage({
  ...
  return (
    <div>
      ...
      <Suspense key={`${from}:${to}`} fallback={<DriversResultsSkeleton />}>
        <DriverResults from={from} to={to} />
      </Suspense>
    </div>
  );
}

async function DriverResults({ from, to }: { from: string; to: string }) {
