import { beforeEach, describe, expect, test, vi } from "vitest";
import { makeQueryBuilder, makeSupabaseMock } from "@/test/supabase-mock";

vi.mock("@/lib/supabase/server", () => ({
  createClient: vi.fn(),
}));

import { createClient } from "@/lib/supabase/server";
import { getPlantSummary } from "@/lib/queries/plants";

describe("getPlantSummary", () => {
  let supabase: ReturnType<typeof makeSupabaseMock>;

  beforeEach(() => {
    supabase = makeSupabaseMock();
    vi.mocked(createClient).mockResolvedValue(supabase as never);
  });

  test("returns rpc data, defaulting null to an empty array", async () => {
    supabase.rpc.mockReturnValue(
      makeQueryBuilder({ data: [{ plant_name: "Petrie" }], error: null })
    );

    expect(await getPlantSummary("2026-08-01", "2026-08-31")).toEqual([
      { plant_name: "Petrie" },
    ]);
    expect(supabase.rpc).toHaveBeenCalledWith("get_plant_summary", {
      date_from: "2026-08-01",
      date_to: "2026-08-31",
    });
  });

  test("throws with the underlying message on error", async () => {
    supabase.rpc.mockReturnValue(
      makeQueryBuilder({ error: { message: "rpc failed" } })
    );

    await expect(getPlantSummary("2026-08-01", "2026-08-31")).rejects.toThrow(
      "Failed to load plant summary: rpc failed"
    );
  });
});
